pipeline{
    ...
    environment {
        ...
        GIT_FOLDER = sh(script:'echo ${GIT_URL} | sed "s/\\s*None\\s*//g"', returnStdout:true).trim()
    ...
    stages {
            ...
            steps {
                ...
                script {
                    ...
                    while(true){
                        ...
                        if (ip.length() >= 7) {
                            echo "Docker Grand Master Public Ip Addess Found: $ip"
        ...
        stage('Deploy the App on Docker Swarm'){
            environment{
                MASTER_INSTANCE_ID = sh(script:'aws ec2 describe-instances --region ${AWS_REGION} --filters Name=tag-value, Values=docker-grand-master Name=tag-value, Values=${AWS_STACK_NAME} --query Reservations[*].Instances[*].[PublicIpAddress] --output text | sed "s/\\s*None\\s*//g"', returnStdout:true).trim()
            }
            steps {
                echo "Cloning and Deploying App on Swarm using Grand Master with Instance Id: ${MASTER_INSTANCE_ID"
                sleep(10)
                sh 'mssh -o UserKnowHostsFile=/dev/null -o StrictHostKeyChecking=no --region ${AWS_REGION} ${MASTER_INSTANCE_ID} docker stack deploy --with-registry-auth -c ${HOME_FOLDER}/${GIT_FOLDER}/docker-compose.yml ${APP_NAME}'
            }
        }
